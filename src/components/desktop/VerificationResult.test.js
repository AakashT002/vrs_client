...
describe('Component: VerificationResults', () => {
  ...
  it('renders the product details', () => {
    expect(wrapper.find('.verification-product__details').exists()).toBe(true);
    expect(wrapper.find('.verification-product__details--gtin').text()).toEqual(
      'GTIN: 13425'
    );
    expect(
      wrapper.find('.verification-product__details--serial-number').text()
    ).toEqual('Serial Number: 1231');
    expect(wrapper.find('.verification-product__details--lot').text()).toEqual(
      'Lot: ABC1234'
    );
    expect(
      wrapper.find('.verification-product__details--expiration').text()
    ).toEqual('Expiration: 21 Mar 2019');
    expect(
      wrapper.find('.verification-product__details--product-name').text()
    ).toEqual('Product: jakafi 60 ct bottle');
  });
});
